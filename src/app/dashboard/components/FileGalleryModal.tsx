      ...
      >
        ...
        <div className="border-b border-gray-700/30 bg-slate-950/70 px-3 sm:px-6 py-3 sm:py-4 flex-shrink-0">
          <input
            ...
            className="w-full rounded-2xl border border-gray-800 bg-slate-950 px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-200 outline-none transition focus:border-blue-500/50 focus:ring-blue-500/10"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="gap-3 sm:gap-4 p-3 sm:p-6">
            {items.length === 0 ? (
              <div className="rounded-2xl border border-gray-800 bg-slate-950/90 p-8 text-center text-slate-400 text-sm">
                {searchValue
              ...
              <div className="space-y-2 sm:space-y-4">
                {items.map((item, index) => {
                  ...
                  return (
                    <div
                      key={`${item.id}-${index}`}
                      className="w-full overflow-hidden rounded-xl border border-slate-700/80 bg-slate-900/70 p-2 transition hover:bg-slate-800/80 sm:rounded-2xl sm:p-4"
                    >
                      <div className="mb-2 sm:mb-3 flex items-start gap-2 sm:gap-4">
                        ...
                        <button
                          ...
                          className={`shrink-0 inline-flex h-8 sm:h-10 w-8 sm:w-10 items-center justify-center rounded-full border border-gray-800 bg-black text-slate-300 transition hover:bg-red-600/10 hover:text-white ${deletingIds.includes(item.id) ? "cursor-not-allowed opacity-60" : ""}`}
                          title={
                      ...
                      {isVideoFile && (
                        <div className="mb-2 sm:mb-4 overflow-hidden rounded-lg sm:rounded-2xl border border-gray-800 bg-black w-full">
                          <video
                            controls
